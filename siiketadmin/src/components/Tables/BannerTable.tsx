...
export const BannerTable = React.memo(
  ({ items, loading, error, onSearchChange }: TableProps) => {
    ...
    const [sortedField, setSortedField] = useState<keyof any>("createdAt");
    ...
    const { searchEvents, items: events } = eventStore();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (formData.id !== "") {
          const result = await updateDataApi(body);
    ...
    return (
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex items-center w-[60%] md:w-[30%] relative"></div>
        </CardHeader>
