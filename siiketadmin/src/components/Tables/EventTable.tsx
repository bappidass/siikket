...
export const EventTable = React.memo(
  ({ items, loading, error, onSearchChange }: TableProps) => {
    ...
    const { searchItems, fetchRecords } = organisationStore();
    ...
    // --- VENDOR FIX ---
    // Event payloads return a `vendors` array of full vendor objects
    // ({id, name, ...}), NOT a `vendor_ids` array of bare ids. Read from
    // `item.vendors` first (no extra network call needed), and fall back
    // to `item.vendor_ids` + a fetch if the API ever sends bare ids.
    const openEditDialog = async (item: any) => {
      if (item.organizer_id != "")
        handleSearchOrganisations(item.organizer_id);

      const vendorObjects = Array.isArray(item.vendors) ? item.vendors : [];
      const vendorIdsFromObjects = vendorObjects.map((v: any) => v.id);
      const vendorIdsFromField = Array.isArray(item.vendor_ids)
        ? item.vendor_ids
        : [];
      const vendorIds =
        vendorIdsFromObjects.length > 0
          ? vendorIdsFromObjects
          : vendorIdsFromField;

      if (vendorObjects.length > 0) {
        setSelectedVendors(
          vendorObjects.map((v: any) => ({
            id: v.id,
            label: v.name,
          })),
        );
        setVendors(vendorObjects);
      } else if (vendorIdsFromField.length > 0) {
        const vendors = await fetchVendors(vendorIdsFromField.join(","));
        setSelectedVendors(
          vendors
            .filter((v: any) => vendorIdsFromField.includes(v.id))
            .map((v: any) => ({
              id: v.id,
              label: v.name,
            })),
      ...
      // --- ZONE FIX ---
      // Zones now come from the real `event_zones` table (returned as
      // `item.zones`, each with a real `id`). Seating types come back
      // with `zone_id` pointing at one of those zones. Since the form
      // works with zones by *position* (zone_index) rather than id
      // (so newly-added, not-yet-saved zones can be referenced too),
      // we resolve each seating type's zone_id to the matching index
      // in the zones array here.
      const zonesFromServer = Array.isArray(item.zones) ? item.zones : [];
      const seatingWithZoneIndex = (item.seating_types || []).map(
        (s: any) => ({
          ...s,
          zone_index: s.zone_id
            ? zonesFromServer.findIndex((z: any) => z.id === s.zone_id)
            : null,
        }),
      );
      ...
      setFormData({
        ...
        seating_types: seatingWithZoneIndex,
        zones: zonesFromServer,
      });
    ...
    const updateSeatType = (index: number, field: string, value: any) => {
    ...
    const removeSeatType = (index: number) => {
      handleFormChange(
        "seating_types",
        formData.seating_types.filter((_: any, i: number) => i !== index),
      );
    };
    ...
    const updateZone = (index: number, value: string) => {
      ...
      handleFormChange("zones", updated);
    };

    // Removing a zone would silently orphan any seating type pointed at
    // it (its zone_index would refer to the wrong zone once indices
    // shift). Clear zone_index on affected seating types and re-index
    // the ones that pointed past the removed zone.
    const removeZone = (index: number) => {
      const updatedZones = formData.zones.filter(
        (_: any, i: number) => i !== index,
      );

      const updatedSeatingTypes = formData.seating_types.map((seat: any) => {
        if (seat.zone_index === index) {
          return { ...seat, zone_index: null };
        }
        if (seat.zone_index !== null && seat.zone_index > index) {
          return { ...seat, zone_index: seat.zone_index - 1 };
        }
        return seat;
      });
      ...
      setFormData((prev) => ({
        ...prev,
        zones: updatedZones,
        seating_types: updatedSeatingTypes,
      }));
    };
                          ...
                          >
                            ...
                            <DropdownMenuContent align="end" className="w-48">
                              ...
                              <DropdownMenuItem
                                className="flex items-center gap-2 cursor-pointer"
                                onClick={() =>
                                  navigate(`/crew-assignment/${item.id}`)
                                }
                              >
          ...
          <DialogContent className="md:max-w-4xl">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <SearchSelect
                    ...
                    value={
                      organisationItems
                        ...
                        .find((e) => e.id === formData.organizer_id) ||
                      undefined
                    }
                ...
                <div>
                  <Label className="mb-2 block">Vendors</Label>
                ...
                <div>
                  <Label className="mb-2 block">Prefix *</Label>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleFormChange("event_date", e.target.value)
                    }
                  />
                </div>
                ...
                <div className="space-y-4 my-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-lg font-semibold">Event Zones</h3>
                  ...
                  {formData.zones.map((zone: any, index: number) => (
                    ...
                    >
                      <div className="flex-1">
                        <Label className="mb-2 block">Zone Name</Label>
                ...
                <div className="md:col-span-2 grid md:grid-cols-2 gap-4">
                  <div>
                    <Label className="mb-2 block">Event Poster *</Label>

                    <ImageUpload
                      folder="events"
                      initialImage={
                        formData.image ? `${PATH_URL}/${formData.image}` : ""
                      }
                  ...
                  <div>
                    ...
                    <ImageUpload
                      ...
                      onUploadComplete={(url) =>
                        handleFormChange("venue_image", url)
                      }
                      onRemoved={() => handleFormChange("venue_image", "")}
                    />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-lg font-semibold">Seating Types</h3>
                  ...
                  {formData.seating_types.map((seat: any, index: number) => (
                    <div
                      key={index}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <h4 className="font-medium">
                            {seat.name || `Seat Type ${index + 1}`}
                          </h4>

                          {seat.price > 0 && (
                            <p className="text-sm text-muted-foreground">
                              ₹{seat.price} • {seat.total_seats || 0} Seats
                            </p>
                          )}
                        </div>
                      ...
                      <div className="grid md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <Input
                            ...
                            value={seat.name}
                            onChange={(e) =>
                              updateSeatType(index, "name", e.target.value)
                            }
                          />
                        </div>
                        ...
                        <div>
                          <Label className="mb-2 block">Available Seats</Label>
                        ...
                        <div>
                          ...
                          <Select
                            value={
                              seat.zone_index !== undefined &&
                              seat.zone_index !== null
                                ? String(seat.zone_index)
                                : ""
                            }
                            onValueChange={(value) =>
                              updateSeatType(
                          ...
                          >
                            ...
                            <SelectContent>
                              {formData.zones.map(
                                (zone: any, zIndex: number) => (
                                  <SelectItem
                                    value={String(zIndex)}
                                    key={zone.id ?? `zone-opt-${zIndex}`}
                                  >
                                    {zone.name || `Zone ${zIndex + 1}`}
                                  </SelectItem>
                                ),
                              )}
                            </SelectContent>
                          ...
                          {formData.zones.length === 0 && (
                            <p className="text-xs text-muted-foreground mt-1">
                              Add a zone above first to link this seating
                              type to it.
                            </p>
                          )}
                        </div>
                      ...
                      <div>
                        <Label className="mb-2 block">Seat Image</Label>

                        <ImageUpload
                          folder="events"
                          initialImage={
                            seat.image ? `${PATH_URL}/${seat.image}` : ""
                          }
                          onUploadComplete={(url) =>
                            updateSeatType(index, "image", url)
                          }
                          onRemoved={() => updateSeatType(index, "image", "")}
                        />
                      </div>
                    </div>
                  ))}
