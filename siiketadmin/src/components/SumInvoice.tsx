...
const TransportSumBillPDF = forwardRef<HTMLDivElement, Props>(
  ({ data }, ref) => {
    if (!data) return;
    if (!data.expand?.invoices_via_master_invoice) {
    ...
    const shipments = data.expand?.invoices_via_master_invoice ? data.expand.invoices_via_master_invoice.map((e: any, i: number) => ({
      sn: i + 1,
      truck: e.expand.transaction?.expand?.fleet?.truck_no ?? "NA",
      bill: e.invoice_number,
      loading: e.expand.transaction?.from_location ?? "NA",
      date: new Date(e.expand.transaction?.date_of_loading).toLocaleDateString("en-GB"),
      unloading: e.expand.transaction?.to_location,
      wt: e.expand.transaction?.loading_point_weight,
      rate: e.expand.transaction?.rate_per_tonne,
      total_amount: e.expand.transaction?.total_amount,
      advance_amount: e.expand.transaction?.advance_amount,
      remaining_balance: e.expand.transaction?.remaining_balance,
      created: new Date(e.expand.transaction?.created).toLocaleDateString("en-GB"),
    })) : [];
    const ar = data.expand?.payments_via_master_invoice ?? [];
    const payments = [...shipments.map((e: any) => ({
      amount: e.advance_amount,
      date: e.created
    })),
    ...ar.map((e: any) => ({
      amount: e.amount,
      date: new Date(e.payment_date).toLocaleDateString("en-GB")
    }))
    ];
...
export default TransportSumBillPDF;