...
const clientStore = create<Store>((set, get) => ({
    items: [],
    loading: false,
    fetchRecords: async () => {
        const state = get();
        ...
        try {
            ...
            const items = await pb.collection("clients").getFullList<RecordModel>({
                sort: "-created",
            });
    ...
    updateRecord: async (id: string, data: any) => {
        try {
            const updatedRecord = await pb.collection("clients").update(id, data);
            const currentItems = get().items;
