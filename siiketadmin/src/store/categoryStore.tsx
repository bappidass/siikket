...
interface Store {
    ...
    createRecord: (data: any) => Promise<{ status: boolean }>;
...
const categoryStore = create<Store>((set, get) => ({
    ...
    createRecord: async (data: any) => {
        ...
        try {
            ...
            if (!user) throw new Error("Not logged in");
            const token = await user.getIdToken();
            const res = await apiFetch(`/api/categories/create`, {
                method: "POST",
                headers: {
                    "Authorization": token,
    ...
    updateRecord: async (data: any) => {
        ...
        try {
            ...
            if (!user) throw new Error("Not logged in");
            const token = await user.getIdToken();
            const res = await apiFetch(`/api/categories/update`, {
                method: "PUT",
                headers: {
                    "Authorization": token,
                    "Content-Type": "application/json"
    ...
    deleteRecord: async (id: string) => {
        ...
        try {
            ...
            if (!user) throw new Error("Not logged in");
            const token = await user.getIdToken();
            const res = await apiFetch(`/api/categories/record/${id}`, {
                method: "DELETE",
                headers: {
                    "Authorization": token,
                },
