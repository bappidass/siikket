...
const eventStore = create<Store>((set, get) => ({
  ...
  fetchRecords: async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/events/partner`, {
        method: "GET",
        headers: {
          Authorization: token,
        },
  ...
  searchRecords: async (q: string) => {
    try {
      ...
      if (!user) throw new Error("Not logged in");
      const token = await user.getIdToken();
      const res = await apiFetch(`/api/events/partner?search=${q}`, {
        // changed
        method: "GET",
      ...
      set({
        ...
        totalPages: resData.total_pages, // note: controller returns total_pages, not totalPages
        page: 1,
        search: q,
      });
      return { status: true };
  ...
  loadMore: async (page: number) => {
    ...
    try {
      ...
      const res = await apiFetch(
        `/api/events/partner?page=${page}&search=${search}`, // changed
